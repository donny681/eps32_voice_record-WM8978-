...
#define I2C_CHECK(a, str)  if((a)) {                                             \
        ESP_LOGE(WM_TAG,"%s:%d (%s):%d", __FILE__, __LINE__, __FUNCTION__, str);      \
        }
...
void WM8978_Init_Read(void)
{
	...
	printf("reg address=1\t%x\r\n",WM8978_Read_Reg(1));
	printf("reg address=1\t%x\r\n",WM8978_Read_Reg(3));
	printf("reg address=43\t%x\r\n",WM8978_Read_Reg(43));
//	printf("reg address=43\t%d\r\n",WM8978_Read_Reg(1));
}
...
uint8_t WM8978_Init(void)
{
	...
	I2C_CHECK(WM8978_Write_Reg(1,0X1B),1);							//R1,MICEN 1(MIC enabled),BIASEN 1(emu on),VMIDSEL[1:0]:11(5K) R1,MICEN设置为1(MIC使能)，BIASEN设置为1(模拟工作，VMIDSEL[1:0]设置为：11（5）)
	I2C_CHECK(WM8978_Write_Reg(2,0X1B0),2);							//R2, ROUT1,LOUT1 output enabled(headphone),BOOSTENR,BOOSTENL enabled.
	I2C_CHECK(WM8978_Write_Reg(3,0X6C),3);							//R3, LOUT2,ROUT2 output enabled(speaker on),RMIX,LMIX enabled

	WM8978_Write_Reg(6,0);						    //R6, MCLK from out
//	WM8978_CLK_Cfg();

	I2C_CHECK(WM8978_Write_Reg(43,1<<4),43);							//R43,INVROUT2 inverted, drive speaker
	I2C_CHECK(WM8978_Write_Reg(47,1<<8),47);							//R47,PGABOOSTL,left MIC got 20 db
	I2C_CHECK(WM8978_Write_Reg(48,1<<8),48);							//R48,PGABOOSTR, right MIC got 20 db
	I2C_CHECK(WM8978_Write_Reg(49,1<<1),49);							//R49,TSDEN, open hot protecting
	I2C_CHECK(WM8978_Write_Reg(10,1<<3),10);							//R10,SOFTMUTE closed,128x sample rate, best SNR
	I2C_CHECK(WM8978_Write_Reg(14,1<<3),14);							//R14,ADC 128x sample rate
